...
namespace ros_control_crtk {
    ...
    void crtkROSHardwareInterface::init(void)
    {
        ...
        // /dvrk/PSM1/operating_state [crtk_msgs/operating_state]
        ...
        // /dvrk/PSM1/measured_js [sensor_msgs/JointState]
        m_measured_js_subscriber = m_node_handle.subscribe("jaw/measured_js", 1,
                                                           &crtkROSHardwareInterface::measured_js_callback, this);

        // add publishers to send servo commands to crtk node
        m_servo_jp_publisher = m_node_handle.advertise<sensor_msgs::JointState>("jaw/servo_jp", 1, false);

        m_number_of_joints = 1;
        m_measured_js.name.resize(m_number_of_joints);
        m_measured_js.name[0] = "jaw";
        ...
        for (size_t i = 0;
            ...
            m_position_joint_interface_jaw
                .registerHandle(hardware_interface::JointHandle(m_joint_state_interface_jaw.getHandle(m_measured_js.name[i]),
                                                                &m_servo_jp.position[i]));
        }

        // register ros control interfaces
        registerInterface(&m_joint_state_interface_jaw);
        registerInterface(&m_position_joint_interface_jaw);
    }
                                            ...
                                            const std::list<hardware_interface::ControllerInfo> & stop_list)
    {
        ...
        for (std::list<hardware_interface::ControllerInfo>::const_iterator iter = stop_list.begin();
            ...
            if (iter->name == "pos_based_pos_traj_controller_jaw") {
                m_servo_jp_interface_running = false;
